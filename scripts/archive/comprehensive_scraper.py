"""
Comprehensive Income Tax Act scraper that extracts:
1. Section information (name, title, description, URL)
2. Related documents (rules, forms, notifications, etc.) for each section
"""
...
def extract_sections_with_related_docs() -> List[Dict[str, Any]]:
    """Extract all sections and their related documents."""
    ...
    # Find all print buttons (these contain section URLs)
    print_buttons = soup.find_all('a', {'onclick': lambda x: x and 'PrintSection' in x})
    
    print(f"Found {len(print_buttons)} sections to process...")
    ...
    for i, button in enumerate(print_buttons):
        ...
        if match:
            ...
            print(f"Processing {normalized_name} ({i+1}/{len(print_buttons)})...")
...
def save_to_excel(sections_data: List[Dict[str, Any]], filename: str = "comprehensive_income_tax_sections.xlsx"):
...
def main():
    ...
    print("Starting comprehensive Income Tax Act scraper...")
    print("This will extract sections and their related documents (rules, forms, etc.)")
    print()
    
    try:
        # Extract all sections with their related documents
        sections_data = extract_sections_with_related_docs()
